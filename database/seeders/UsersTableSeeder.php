<?php
namespace Database\Seeders;

use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Hash;
use App\Helpers\Qs;
use Illuminate\Support\Str;

class UsersTableSeeder extends Seeder
{

    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        // Check if admin accounts already exist
        $existingAdmin2 = DB::table('users')->where('username', 'admin2')->first();
        $existingAdmin3 = DB::table('users')->where('username', 'admin3')->first();
        $existingLibrarian = DB::table('users')->where('username', 'librarian')->first();
        
        $password = Hash::make('cj');
        
        // Add Admin Second if it doesn't exist
        if (!$existingAdmin2) {
            DB::table('users')->insert([
                'name' => 'Admin Second',
                'email' => 'admin2@admin.com',
                'password' => $password,
                'user_type' => 'admin',
                'username' => 'admin2',
                'code' => strtoupper(Str::random(10)),
                'remember_token' => Str::random(10),
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        }
        
        // Add Admin Third if it doesn't exist
        if (!$existingAdmin3) {
            DB::table('users')->insert([
                'name' => 'Admin Third',
                'email' => 'admin3@admin.com',
                'password' => $password,
                'user_type' => 'admin',
                'username' => 'admin3',
                'code' => strtoupper(Str::random(10)),
                'remember_token' => Str::random(10),
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        }
        
        // Add Librarian account if it doesn't exist
        if (!$existingLibrarian) {
            DB::table('users')->insert([
                'name' => 'Librarian',
                'email' => 'librarian@librarian.com',
                'password' => $password,
                'user_type' => 'librarian',
                'username' => 'librarian',
                'code' => strtoupper(Str::random(10)),
                'remember_token' => Str::random(10),
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        }
    }

    protected function createNewUsers()
    {
        $password = Hash::make('cj'); // Default user password

        $d = [

            ['name' => 'CJ Inspired',
                'email' => 'cj@cj.com',
                'username' => 'cj',
                'password' => $password,
                'user_type' => 'super_admin',
                'code' => strtoupper(Str::random(10)),
                'remember_token' => Str::random(10),
            ],

            ['name' => 'Admin KORA',
            'email' => 'admin@admin.com',
            'password' => $password,
            'user_type' => 'admin',
            'username' => 'admin',
            'code' => strtoupper(Str::random(10)),
            'remember_token' => Str::random(10),
            ],

            ['name' => 'Admin Second',
            'email' => 'admin2@admin.com',
            'password' => $password,
            'user_type' => 'admin',
            'username' => 'admin2',
            'code' => strtoupper(Str::random(10)),
            'remember_token' => Str::random(10),
            ],

            ['name' => 'Admin Third',
            'email' => 'admin3@admin.com',
            'password' => $password,
            'user_type' => 'admin',
            'username' => 'admin3',
            'code' => strtoupper(Str::random(10)),
            'remember_token' => Str::random(10),
            ],

            ['name' => 'Teacher Chike',
                'email' => 'teacher@teacher.com',
                'user_type' => 'teacher',
                'username' => 'teacher',
                'password' => $password,
                'code' => strtoupper(Str::random(10)),
                'remember_token' => Str::random(10),
            ],

            ['name' => 'Parent Kaba',
                'email' => 'parent@parent.com',
                'user_type' => 'parent',
                'username' => 'parent',
                'password' => $password,
                'code' => strtoupper(Str::random(10)),
                'remember_token' => Str::random(10),
            ],

            ['name' => 'Accountant Jeff',
                'email' => 'accountant@accountant.com',
                'user_type' => 'accountant',
                'username' => 'accountant',
                'password' => $password,
                'code' => strtoupper(Str::random(10)),
                'remember_token' => Str::random(10),
            ],
        ];
        DB::table('users')->insert($d);
    }

    protected function createManyUsers(int $count)
    {
        $data = [];
        $user_type = Qs::getAllUserTypes(['super_admin', 'librarian', 'student']);

        for($i = 1; $i <= $count; $i++){

            foreach ($user_type as $k => $ut){

                $data[] = ['name' => ucfirst($user_type[$k]).' '.$i,
                    'email' => $user_type[$k].$i.'@'.$user_type[$k].'.com',
                    'user_type' => $user_type[$k],
                    'username' => $user_type[$k].$i,
                    'password' => Hash::make($user_type[$k]),
                    'code' => strtoupper(Str::random(10)),
                    'remember_token' => Str::random(10),
                ];

            }

        }

        DB::table('users')->insert($data);
    }
}
