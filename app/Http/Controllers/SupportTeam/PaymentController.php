...
class PaymentController extends Controller
{
    ...
    public function manage($class_id = NULL)
    {
        ...
        if($class_id){
            $d['students'] = $st = $this->student->getRecord(['my_class_id' => $class_id])->get()->sortBy('user.name')->load(['user', 'my_parent']);
            if($st->count() < 1){
            ...
            foreach ($st as $s) {
                $prs = $this->pay->getRecord(['student_id' => $s->user_id, 'year' => $this->year])->get();
                $totalBalance = $prs->sum(function ($pr) {
                    $bal = $pr->balance ?? ($pr->payment->amount - ($pr->amt_paid ?? 0));
                    return $pr->paid ? 0 : max(0, $bal);
                });
                $d['student_balances'][$s->user_id] = $totalBalance;
            }
        }
    ...
    /**
     * Daily payment report for accountant to submit to admin.
     */
    public function dailyReport()
    {
        $today = now()->format('Y-m-d');
        $receipts = Receipt::whereDate('created_at', $today)
            ->with(['pr.payment', 'pr.student'])
            ->orderBy('created_at', 'desc')
            ->get();

        $totalCollected = $receipts->sum('amt_paid');

        return view('pages.support_team.payments.daily_report', [
            'receipts' => $receipts,
