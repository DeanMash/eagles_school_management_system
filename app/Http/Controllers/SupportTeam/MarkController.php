...
class MarkController extends Controller
{
    ...
    {
        $student_id = Qs::decodeHash($student_id) ?: $student_id;
    ...
    {
        // Decode the hashed student_id
        $decoded_student_id = Qs::decodeHash($student_id);
        
        /* Prevent Other Students/Parents from viewing Result of others */
        if(Auth::user()->id != $decoded_student_id && !Qs::userIsTeamSAT() && !Qs::userIsMyChild($decoded_student_id, Auth::user()->id)){
        ...
        if(Mk::examIsLocked() && !Qs::userIsTeamSA()){
            ...
            if(!$this->checkPinVerified($decoded_student_id)){
                return redirect()->route('pins.enter', $student_id);
            }
        }

        if(!$this->verifyStudentExamYear($decoded_student_id, $year)){
            return $this->noStudentRecord();
        }

        $wh = ['student_id' => $decoded_student_id, 'year' => $year ];
        $d['marks'] = $this->exam->getMark($wh);
        $d['exam_records'] = $exr = $this->exam->getRecord($wh);
        $d['exams'] = $this->exam->getExam(['year' => $year]);
        $d['sr'] = $this->student->getRecord(['user_id' => $decoded_student_id])->first();
        $d['my_class'] = $mc = $this->my_class->getMC(['id' => $exr->first()->my_class_id])->first();
        $d['class_type'] = $this->my_class->findTypeByClass($mc->id);
        $d['subjects'] = $this->my_class->findSubjectByClass($mc->id)->load('teacher');
        $d['year'] = $year;
        $d['student_id'] = $decoded_student_id;
    ...
    public function print_view($student_id, $exam_id, $year)
    {
        ...
        /* Prevent Other Students/Parents from viewing Result of others */
        if(Auth::user()->id != $decoded_student_id && !Qs::userIsTeamSA() && !Qs::userIsMyChild($decoded_student_id, Auth::user()->id)){
            return redirect(route('dashboard'))->with('pop_error', __('msg.denied'));
        }
        ...
        $wh = ['student_id' => $decoded_student_id, 'exam_id' => $exam_id, 'year' => $year ];
        ...
        $d['tex'] = 'tex'.$exam->term;
        $d['sr'] = $sr =$this->student->getRecord(['user_id' => $decoded_student_id])->first();
        $d['class_type'] = $this->my_class->findTypeByClass($mc->id);
        $d['subjects'] = $this->my_class->findSubjectByClass($mc->id)->load('teacher');
