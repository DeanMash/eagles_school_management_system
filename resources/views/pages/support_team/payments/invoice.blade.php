    ...
    <div class="card">
        <div class="card-header header-elements-inline">
            <h6 class="card-title font-weight-bold">
                <a href="{{ route('payments.manage') }}" class="text-muted mr-2" title="Back to Student Payments"><i class="icon-arrow-left8"></i></a>
                Make Payment — {{ $sr->user->name }}
                <small class="d-block text-muted mt-1">Payments are deducted from the set fee. Balance updates after each payment.</small>
            </h6>
            <div class="header-elements">
                <a href="{{ route('payments.manage') }}" class="btn btn-outline-secondary btn-sm">
                    <i class="icon-arrow-left8"></i> Back to Student Payments
                </a>
                {!! Qs::getPanelOptions() !!}
            </div>
        </div>
                    ...
                    @foreach($uncleared as $uc)
                        <tr>
                            ...
                            <td>
                             ...
                             <div class="row">
                                 <div class="col-md-7">
                                     <input min="0.01" step="0.01" max="{{ $uc->balance ?: $uc->payment->amount }}" id="val-{{ Qs::hash($uc->id) }}" class="form-control" required placeholder="Amount" title="Enter amount to pay" name="amt_paid" type="number">
                                 </div>
                                 <div class="col-md-5">
